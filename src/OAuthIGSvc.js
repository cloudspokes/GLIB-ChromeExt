...
class OAuthIGService {
    ...
    constructor(getStorage, setStorage, setupTokenResolver, requestFunc, config, logger) {
        ...
        this.setupTokenResolver = function(cb) {
            setupTokenResolver(cb);
        };
        this.requestFunc = function(uri) {
            requestFunc(uri);
        };
        this.config = config;
            ...
            const resolver = (tokenUri) => {
                if (tokenUri) {
                    ...
                    if (resp.error) {
                        ...
                        //parsedJwt.bearer = 'Bearer "' + resp.acc  ess_token + '"';
...
function extractParamsFromURIFragment(q) {
    ...
    var e,
        ...
        }; // decode extracted keyvalue pairs
