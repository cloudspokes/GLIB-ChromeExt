...
$(document).ready(function(){
	...
	$(document).on("click",".remove",function(e){
		...
		chrome.storage.local.get("repoMap", function (result){
			...
			$(self).parent().fadeOut(300, function(){ $(self).parent().remove();});
	...
	$("#addMass").click(function(){
		var self = this;
		vex.dialog.open({
			message: 'Enter TopCoder project id and GitHub repo separated by '+ADD_MASS_DELIMETER+' (one mapping on each line)',
			input: "<textarea rows=\"7\" placeholder=\"PROJECT_ID "+ADD_MASS_DELIMETER+" REPO_URL\nPROJECT_ID "+ADD_MASS_DELIMETER+" REPO_URL\" name=\"mappings\"></textarea>",
			buttons: [
			...
			callback: function(data) {
				if (data === false) {
					...
					for(var i=0;i<lines.length;i++){
						...
						mapArr.push({"projectId":projectId,"repoURL":gitRepo});
					}
